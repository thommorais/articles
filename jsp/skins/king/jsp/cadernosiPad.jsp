...
<div class="dockindice">
    ...
    %>
    <li class="indice" title="<fmt:message key= 'site.cliquepagina'/>"><span class="dock_icone">&raquo;</span>
        <a href="javascript:void(0)" onclick="abrePaginaIndice('<%= anchor%>',<%= index %>); return false;">
            <c:choose>
    ...
    <li class="indice" title="<%= child.getNome()%>" style="display:block;float:left;width:96%;height:auto;margin-right:5px;">
        ...
        <a href="javascript:void(0)" onclick="abrePaginaIndice('<%= anchor1%>',<%= index1 %>); return false;">
            <c:choose>
                <c:when test="${sessionScope['javax.servlet.jsp.jstl.fmt.locale.session'] eq 'en'}">
                    <%= child.getNomeEn()%>
                </c:when>
