...
function openZoomMode(item, e) {
    ...
    if (isLandscape()) {
        ...
        if (pindex > 1 && ponto > 250 && pindex < (total - 1)) {
            pindex++;
        }
    ...
    jQuery(image).load(function () {
        ...
        loaded();
        _gaq.push(['_trackEvent', document.title, 'Zoom em iphone', 'Pagina ' + page]);
        //console.log('links'+pindex);
        setTimeout(function () {
    ...
    for (var p = 0; p < pfpages.length; p++) {
        if (changes == 1) {
            if (parseInt(pfpages[p][0]) == parseInt(pindex)) {
                ...
                if (p < (pfpages.length - 1))
                    loadImageToDiv(pfpages[p + 1]);
                if (p < (pfpages.length - 2))
                    loadImageToDiv(pfpages[p + 2]);
                updatePageChange();
                if (myScroll != null) {
                    setTimeout(function () {
                        myScroll.refresh();
            ...
            if (parseInt(pfpages[p][0]) == parseInt(novoindex)) {
                loadImageToDiv(pfpages[p]);
                if (p > 0)
                    loadImageToDiv(pfpages[p - 1]);
                if (p < (pfpages.length - 1))
...
function loadImageToDiv(item) {
    ...
    if (html == null || (html != null && html == '') || (html != null && html.indexOf("empty") != -1)) {
        jQuery("#pag" + index).html("<img src='" + base + "flip/jornal/skins/mobile/load.gif' align='center' id='carregador' style='width:50px;height:50px;max-width:50px;max-height:50px;' width='50px' height='50px'/>");
        var img = new Image();
...
function resetaPagina() {
    ...
    jQuery("#edicoes_barra .free").html("");
    lockdock = false;
...
function addAreaLinkZoom(pagId, x, y, w, h, index, link, px, py, linkId, bw, bh, tipo, tamanho, popup, icone) {
    var img = jQuery("#scroll-content img");
    if (img) {
        x = parseInt(x);
        y = parseInt(y);
        w = parseInt(w);
        ...
        newh = newh + fator;
        newh = newh + (newh / 2);
        newy = newy + (innerScroll.y);
        newx = newx + (innerScroll.x);
        var overlay = ("#overlayzoom" + linkId);
        if (jQuery(overlay).length) {
            jQuery(overlay).css('left', newx);
            jQuery(overlay).css('top', newy);
            jQuery(overlay).css('width', neww);
            ...
            if (link.indexOf("abreModal") == -1 || popup == 'true') {
                var action = "window.open('" + link + "');";
                if (link.indexOf("anch") != -1) {
                    var p = jQuery("." + link).attr('id').replace("pag", "");
                    action = "abrePaginaPorAnchor(" + p + ");";
                }
                overlay = "<div class=\"overlay\" id=\"overlayzoom" + linkId + "\" style='position:absolute;left:" + newx + "px;top:" + newy + "px;width:" + neww + "px;height:" + newh + "px' onclick=\"" + action + "\">&nbsp;</div>";
                jQuery("#scroll-content").append(overlay);
            } else {
                overlay = "<div class=\"overlay\" id=\"overlayzoom" + linkId + "\" style='position:absolute;left:" + newx + "px;top:" + newy + "px;width:" + neww + "px;height:" + newh + "px' onclick=\"jQuery('#menufechar').click();" + link + "\">&nbsp;</div>";
                jQuery("#scroll-content").append(overlay);
...
function openEditionMode(p) {
    var html = '<div id="header"><li class="header">Edi√ß√µes anteriores</li></div>';
    var pageAnchor = getCurrentEdition();
    if (!p)
        p = 0;
    var n = isLandscape() ? 4 : 8;
    var height = n == 8 ? 308 : 200;
    jQuery("#tab_left").css("height", height + "px");
    jQuery("#dock > li ul").css("height", height + "px")
    jQuery("#dock > li ul").css("max-height", height + "px")
    jQuery(".dockedicao").css("height", height + "px");

    var url = getPagesLink('edicoesIphone') + '?idForm=' + pageAnchor + '&linkedicao=' + linkedicao + '&pagina=' + p + '&n=' + n;
...
function loadDock(name) {
    ...
    if (name == 'indice_barra') {
        if (jQuery('#indice_barra .free').html().length == 0) {
            ...
            var html = '<div id="header"><li class="header">√çndice</li></div>';
            var pageAnchor = getCurrentEdition();
            ...
            if (name == 'produtos_barra') {
                ...
                if (name == 'sumario_barra') {
                    if (jQuery('#sumario_barra .free').html().length == 0) {
                        var html = '<div id="header"><li class="header">P·ginas</li></div>';
                        var pageAnchor = getCurrentEdition();
...
var addToHomeConfig = {
    ...
    expire: 2,
    touchIcon: true,
    message: "Instale o aplicativo em seu %device. Clique em %icon e escolha 'Adicionar ‡† tela de inÌcio'"
};
