...
function openZoomMode(item, e) {
    ...
    if (isLandscape()) {
        jQuery("#zoomarea").html("<img src='" + base + "flip/jornal/skins/mobile/load.gif' id='loader' align='center' style='margin-top:30px;top:30px;max-width:50px;width:50px;height:50px;max-height:50px;' width='50px' height='50px'/>");
    } else {
        jQuery("#zoomarea").html("<img src='" + base + "flip/jornal/skins/mobile/load.gif' id='loader' align='center' style='margin-top:90px;top:90px;max-width:50px;width:50px;height:50px;max-height:50px;'  width='50px' height='50px'/>");
    }
    ...
    //var img = img.replace('.jpg', '_mob.jpg');
    var image = new Image();
    jQuery(image).load(function () {
        jQuery("#loader").hide();
        jQuery("#zoomarea").append("<div id='scroll-content'></div>");
        jQuery("#scroll-content").append(this);
        _gaq.push(['_trackEvent', document.title, 'Zoom em android', 'Pagina ' + page]);
        loaded();
        setTimeout(function () {
...
function openZoomMode2(item, e) {
    ...
    if (img != '') {
        ...
        jQuery.post(url, function (data) {
            ...
            var imagetop = 0;
            var imagepos = -1;
            var imgwidth = land ? 501 : 391;
            var imgheight = land ? 667 : 537;
...
function clearAllPages() {
    jQuery('li[id^="pag"]').each(function (index) {
        jQuery(this).html('<span id="empty" class="loadimg"></span>');
    });
    lastpage = 0;
}
...
function configure() {
    ...
    //jQuery("#ext-gen1024").click(function (that, e) {
    //    openZoomMode(lastobject, e);
    //});
    document.addEventListener('touchmove', function (e) {
        e.preventDefault();
...
function init() {
    ...
    jQuery("#dock li").click(function (e) {
        ...
        if (lockdock == true) {
            ...
            if (this.id != lockbar) {
                ...
                jQuery("#dock").find("ul.free").css("width", w);
            }
        } else {
            loadDock(this.id);
            jQuery(this).find("ul").animate({
    ...
    var width = jQuery(window).width();
//    AJUSTE PARA APARECERA AS ABAS LATERAIS
//    EM TODOS OS DISPOSITIVOS INDEPENDENTE DO TAMANHO
//    if (width<600) {
//        jQuery("#tab_left").hide();
//    }
    if (page > 0 || ipg != null) {
    ...
    jQuery(window).on("orientationchange", function (e) {
        ...
        clearAllPages();
        setTimeout(updateOrientation, 2000);
        setTimeout(function () {
            resetaPagina();
        }, 5000);
        loadImages();
        jQuery("#rotacionando").fadeOut(10000);
    });
}
var editingText = '';
...
function reloadLinksZoom(index) {
    if (pflinks.length > 0) {
                ...
                if (y > element.top && y < element.top + element.height
                    ...
                    if (element.link.indexOf("abreModal") == -1) {
                        ...
                        if (element.link.indexOf("anch") != -1) {
                            var p = jQuery("." + element.link).attr('id').replace("pag", "");
        ...
        for (var p = 0; p < pflinks.length; p++) {
            if (index == null || (pflinks[p] != null && pflinks[p][0] == index)) {
                addAreaLinkZoom(pflinks[p][0], pflinks[p][1], pflinks[p][2], pflinks[p][3], pflinks[p][4], index, pflinks[p][5], pflinks[p][6], pflinks[p][7], pflinks[p][8], pflinks[p][9], pflinks[p][10], pflinks[p][11], pflinks[p][12], pflinks[p][13], pflinks[p][14]);
            }
        }
    }
}
        ...
        if (jQuery(overlay).length) {
            ...
            if (tipo == 'V' || popup == 'true') {
                ...
                if (link.indexOf("anch") != -1) {
                    var p = jQuery("." + link).attr('id').replace("pag", "");
                    action = "abrePaginaPorAnchor(" + p + ");";
                }
                if (icone != null && icone != '') {
                    overlay = "<div class=\"overlaytransp\" id=\"overlay" + linkId + "\" style='position:absolute;left:" + newx + "px;top:" + newy + "px;width:" + neww + "px;height:" + newh + "px' onclick=\"" + action + "\">";
...
function addVideoInline(pagId, x, y, w, h, index, src, px, py, linkId, bw, bh) {
    ...
    if (jQuery(img).length) {
        ...
        if (jQuery(overlay).length) {
            ...
            src = src + '?date=' + new Date();
            var div = "<div id=\"overvideo" + linkId + "\" style=\"position:absolute;left:" + newx + "px;top:" + newy + "px; width:" + neww + "px; height:0px;\">&nbsp;";
            div = div + '<video width="' + neww + 'px" height="' + newh + 'px" controls poster=\"http://flip.siteseguro.ws/loading.gif\">';
            div = div + '<source src="' + base + src + '" type="video/mp4" codecs=\"avc1.42E01E, mp4a.40.2\" />';
            div = div + '</video>';
            div = div + "</div>"
            jQuery("#pag" + pagId + " #thepage").append(div);
        }
    }
}
var whatsppobj = null;
function updatePageChange() {
    ...
    validateRecomment();
    if (jQuery(".wa_btn_s")) {
        jQuery(".wa_btn_s").attr('data-href', base + linkedicao + '?pg=' + getCurrentPage());
    }
    //WASHAREBTN.setButtonAttributes(jQuery('iframe'));
}
...
function openEditionMode(p) {
    var html = '<div id="header"><li class="header">Ediçoes anteriores</li></div>';
    ...
    if (name == 'indice_barra') {
        if (jQuery('#indice_barra .free').html().length == 0) {
            ...
            jQuery.post(url, function (data) {
                ...
                jQuery(".dockindice").css("height", jQuery("#imagegallery").css("height"));
                jQuery('#indice_barra .free').css("height", jQuery("#imagegallery").css("height"));
            });
        }
    } else {
        if (name == 'edicoes_barra') {
            ...
            openEditionMode();
            jQuery(".dockedicao").css("height", jQuery("#imagegallery").css("height"));
            jQuery('#edicoes_barra .free').css("height", jQuery("#imagegallery").css("height"));
            jQuery(".dockedicao").css("width", jQuery(window).width() - 80);
            //}
        } else {
            if (name == 'produtos_barra') {
                //if (jQuery('#produtos_barra .free').html().length==0) {
                openProdutosMode();
                jQuery(".docprodutos").css("height", jQuery("#imagegallery").css("height"));
                jQuery('#produtos_barra .free').css("height", jQuery("#imagegallery").css("height"));
                jQuery(".dockproduto").css("width", jQuery(window).width() - 80);
                //}
            } else {
                if (name == 'sumario_barra') {
                    ...
                    jQuery.post(url, function (data) {
                        ...
                        jQuery(".dockpagina").css("height", jQuery("#imagegallery").css("height"));
                        jQuery('#sumario_barra .free').css("height", jQuery("#imagegallery").css("height"));
                        jQuery(".dockpagina").css("width", jQuery(window).width() - 80);
                    });
                    //}
                }
            }
...
function isLandscape() {
    return orientation == 'landscape' || jQuery(window).width() > jQuery(window).height();
}
