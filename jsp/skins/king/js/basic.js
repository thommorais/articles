...
jQuery(function ($) {
    ...
    jQuery('.multimidia_fr').click(function () {
        MegaZine.firstPage();
        if (dataLayer != null) {
            dataLayer.push({'event': 'eventTracking', 'eventCategory': 'Cliques Flip - ' + sigla, 'eventAction': 'Primeira Pagina', 'eventLabel': getDataAnalytics()});
        }
    });
    ...
    jQuery('.multimidia_prev').click(function () {
        MegaZine.prevPage();
        if (dataLayer != null) {
            dataLayer.push({'event': 'eventTracking', 'eventCategory': 'Cliques Flip - ' + sigla, 'eventAction': 'Retornou Pagina', 'eventLabel': getDataAnalytics()});
        }
    });
    ...
    jQuery('.multimidia_next').click(function () {
        MegaZine.nextPage();
        if (dataLayer != null) {
            dataLayer.push({'event': 'eventTracking', 'eventCategory': 'Cliques Flip - ' + sigla, 'eventAction': 'Avancou Pagina', 'eventLabel': getDataAnalytics()});
        }
    });
    ...
    jQuery('.multimidia_ff').click(function () {
        MegaZine.lastPage();
        if (dataLayer != null) {
            dataLayer.push({'event': 'eventTracking', 'eventCategory': 'Cliques Flip - ' + sigla, 'eventAction': 'Ultima Pagina', 'eventLabel': getDataAnalytics()});
        }
    });
...
function openEditionMode(p) {
    ...
    var url = base + '/flip/abaEdicoes.do?idForm=' + pageAnchor + '&linkedicao=' + linkedicao + '&pagina=' + page;
...
function getDataAnalytics(){
    var pagina=MegaZine.getCurrentPage();    
    var lastIndex="Capa";
    for(var i=0;i<indexList.length;i++){
        var cPage=indexList[i][0];     
        if(parseInt(pagina)>parseInt(cPage)){
            lastIndex=indexList[i][1];
        }
    }
    
    var dt=dataAnalytics+" - pag "+pagina+" e "+(pagina+1)+" - "+lastIndex;   
    return dt;
}